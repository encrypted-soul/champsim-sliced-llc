...
bool L2C::readlike_miss(PACKET& handle_pkt)
{
  ...
  if (mshr_entry != MSHR.end()) // miss already inflight
  {
    ...
    if (!is_read)
      sliced_lower_level->add_pq(&handle_pkt);
    else
      sliced_lower_level->add_rq(&handle_pkt);
  }
...
bool L2C::filllike_miss(std::size_t set, std::size_t way, std::size_t slice, PACKET& handle_pkt)
{
  ...
  BLOCK& fill_block = block[set * NUM_WAY + way];
  bool evicting_dirty = !bypass && (sliced_lower_level != NULL) && fill_block.dirty;
  ...
  if (!bypass) {
    if (evicting_dirty) {
      ...
      writeback_packet.fill_level = sliced_lower_level->fill_level;
      ...
      auto result = sliced_lower_level->add_wq(&writeback_packet);
      if (result == -2)
        return false;
    }
...
int L2C::prefetch_line(uint64_t pf_addr, bool fill_this_level, uint32_t prefetch_metadata)
{
  ...
  pf_packet.fill_level = (fill_this_level ? fill_level : sliced_lower_level->fill_level);
